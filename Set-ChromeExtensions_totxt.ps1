...
Get-Date | Out-File -FilePath Out-File -FilePath C:\ProgramData\Quest\KACE\user\chrome_extensions.txt;
    ...
    If(Test-Path ($_.profileimagepath + "\appdata\local\google\chrome\user data\default\extensions")){
        ...
        ForEach ($CPF in $ChromeProfileFolder){
            ...
            $Name = (Get-Content $ManifestFile | Where-Object {$_ -like '*"name":*'})  | foreach{
                ...
                #Write to c:\chrome_extensions.txt
                $dataString = "$j,$NameToRecord,$Userpath,$ManifestFileFolder,$FolderDate,$Version"
                #Write-Host $dataString
                $dataString| Out-File -FilePath C:\ProgramData\Quest\KACE\user\chrome_extensions.txt -Append;
                $j=$j+1
            }
        }
    }
}
$wmiClass = get-wmiobject -Namespace root/$Namespace -Class $Class
$jsonOutput = $wmiClass | ConvertTo-Json
try {
$jsonOutput | Out-File -FilePath c:\Programdata\Quest\KACE\user\chrome_extensions.json
